...
3. "urgency": "red" (critical/urgent) | "yellow" (attention/awareness) | "grey" (info/reference).
4. "threadTopic": Normalized English topic slug.
5. "requiresReply": boolean (false if sent email, automated bot, or alert; true ONLY if incoming email from real client expecting answer).
      ...
      if (res.ok) {
        ...
        if (text) {
          ...
          return {
            ...
            urgency: isSent ? 'grey' : (['red', 'yellow', 'grey'].includes(parsed.urgency) ? parsed.urgency : 'grey'),
