...
1. "author": Clean sender/service name (e.g. "Manychat", "Vercel", "Google", "Lloyd Alvarez", "You (Sent)"). Max 2 words. NO EMOJIS.
2. "geminiTitle": Sharp, concise, INVENTED title strictly in ENGLISH. DO NOT simply copy the original subject. Summarize the core meaning in max 6 words. NO EMOJIS.
3. "urgency":
   - "spam": STUFF TO DISCARD COMPLETELY (marketing promotions, store sales, shopping ads, retail discounts, flight deals, generic newsletters, junk). If spam, return "urgency": "spam".
   - "red": Critical/urgent studio actions (server outage, deploy failed, payment declined, subscription expired, security alert).
   - "yellow": Attention/awareness (client brief, scheduled maintenance, customer inquiry).
   - "grey": Useful reference only (security 2FA events, password changes, official invoices, receipts, relevant tech conferences/events).
4. "threadTopic": Normalized English topic slug (NO EMOJIS).
5. "requiresReply": boolean (false if sent email, bot, alert; true ONLY if real human client expecting response).
6. "hasActionableDeadline": boolean (true if deadline, payment expiration, or concrete action required).
7. "suggestedTodoTitle": Clear Russian action directive if actionable (e.g. "Исправить ошибку деплоя Vercel"). No colons, ampersands, or emojis.
8. "cleanBody": Structured, high-density, ULTRA-COMPACT readable digest strictly in ENGLISH. DO NOT use multiple line breaks. DO NOT use emojis. Strip out all spaces between paragraphs, use only single newline character. Remove legal footers, unsubscribe links, duplicate subjects, and tracking noise. Preserve key URLs cleanly.
...
export function groupMessagesIntoThreads(messages) {
  ...
  sorted.forEach(msg => {
    ...
    const normCounterparty = counterpartyEmail.toLowerCase().replace(/[^a-z0-9]/g, '');
