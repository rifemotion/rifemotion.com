...
export async function getDb() {
  try {
    ...
    const cloudContextItems = [
      ...(Array.isArray(cloudData?.userContext?.items) ? cloudData.userContext.items : []),
      ...(Array.isArray(dataFromFile?.userContext?.items) ? dataFromFile.userContext.items : []),
      ...(Array.isArray(global.cachedDb?.userContext?.items) ? global.cachedDb.userContext.items : [])
    ];
    const uniqueContextItems = Array.from(new Set(cloudContextItems));

    const mergedMessages = [
      ...(Array.isArray(cloudData?.messages) ? cloudData.messages : []),
      ...(Array.isArray(dataFromFile?.messages) ? dataFromFile.messages : []),
      ...(Array.isArray(global.cachedDb?.messages) ? global.cachedDb.messages : [])
    ];
    const msgMap = new Map();
    mergedMessages.forEach(m => { if (m && m.id) msgMap.set(m.id, m); });
    const finalMessages = Array.from(msgMap.values()).sort((a, b) => new Date(b.date) - new Date(a.date));

    const mergedAccounts = [
      ...(Array.isArray(cloudData?.connectedGmailAccounts) ? cloudData.connectedGmailAccounts : []),
      ...(Array.isArray(dataFromFile?.connectedGmailAccounts) ? dataFromFile.connectedGmailAccounts : []),
      ...(Array.isArray(global.cachedDb?.connectedGmailAccounts) ? global.cachedDb.connectedGmailAccounts : [])
    ];
    const accMap = new Map();
    mergedAccounts.forEach(a => { if (a && a.email) accMap.set(a.email.toLowerCase(), a); });
    ...
    const result = {
      ...
      connectedGmailAccounts: finalAccounts,
      userContext: { items: uniqueContextItems },
...
export async function writeDb(data) {
  // PROTECT CONNECTED ACCOUNTS FROM EVER BEING ACCIDENTALLY OVERWRITTEN
  if (!Array.isArray(data.connectedGmailAccounts) || data.connectedGmailAccounts.length === 0) {
    if (Array.isArray(global.cachedDb?.connectedGmailAccounts) && global.cachedDb.connectedGmailAccounts.length > 0) {
      data.connectedGmailAccounts = global.cachedDb.connectedGmailAccounts;
    }
  }
  try {
