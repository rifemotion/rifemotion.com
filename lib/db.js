...
export async function getDb() {
  try {
    ...
    // Cumulative union: NEVER delete existing feedback on refresh
    ...
    // Merge all sources cumulatively
    cloudFeedback.forEach(item => { if (item && item.id) feedbackMap.set(item.id, item); });
    fileFeedback.forEach(item => { if (item && item.id) feedbackMap.set(item.id, item); });
    memoryFeedback.forEach(item => { if (item && item.id) feedbackMap.set(item.id, item); });
